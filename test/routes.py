...
from .utils import allowed_file, resize_image, nocache
from .models import Image, db
from test.ai import detect_and_classify
...
upload_folder = r'C:\caps\myproject\uploads'
...
@main_bp.route('/upload_web', methods=['POST'])
@login_required
def upload_web():
    ...
    if allowed_file(file.filename):
        ...
        try:
            ...
            return render_template("result.html",
                                   error=None,
                                   result=result,
                                   file_path=url_for('main.uploaded_file', filename=unique_filename))
        except Exception as e:
            return render_template("result.html",
...
def images():
    ...
    processed_entries = [{
        ...
        'result': img.result,
        'url': url_for('main.uploaded_file', filename=os.path.basename(img.file_path), _external=True)
...
@nocache
@main_bp.route('/delete/<int:image_id>', methods=['DELETE'])
